"use client";
import { Search, ShieldAlert, FlaskConical, Stethoscope, Info, Camera, Zap } from "lucide-react";
...
export default function DiseaseGuideUI() {
  ...
  const filteredData = DISEASES.filter(d => 
      ...
      <section className="flex flex-col md:flex-row gap-6 items-center bg-[var(--surface)]/30 p-4 rounded-2xl border border-[var(--border)]/50 shadow-inner">
        ...
        >
          ...
          <div className="whitespace-nowrap">
            <p className="text-[9px] font-black uppercase tracking-widest opacity-80 leading-none">AI Detector</p>
            <h4 className="font-black text-md mt-1 tracking-tight">ছবি তুলে রোগ শনাক্ত করুন</h4>
          </div>
        </motion.div>
      </section>
              ...
              >
                <div className="relative w-full md:w-1/2 min-h-[350px] md:min-h-full rounded-2xl overflow-hidden group shadow-2xl border border-white/5">
                  ...
                  <div className="absolute top-6 left-6 flex flex-col gap-2">
                    ...
                    <span className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border self-start ${
                      ...
                      {disease.riskLevel === 'high' ? 'High Risk' : 'Medium Risk'}
                    </span>
                  </div>
                </div>
                ...
                <div className="w-full md:w-1/2 flex flex-col justify-between space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      ...
                      <ul className="text-sm space-y-2 opacity-50 font-bold">
                        {disease.causes.map((c, i) => <li key={i} className="flex gap-2"><span>•</span>{c}</li>)}
                      </ul>
                    </div>
                    <div className="space-y-3">
                      ...
                      <ul className="text-sm space-y-2 opacity-50 font-bold">
                        {disease.prevention.map((p, i) => <li key={i} className="flex gap-2"><span>•</span>{p}</li>)}
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="p-5 rounded-2xl bg-[var(--primary)]/5 border border-[var(--primary)]/10">
                      <div className="flex items-center gap-3 mb-2">
                        <Stethoscope className="text-[var(--primary)]" size={20} />
                        <span className="text-[10px] font-black uppercase tracking-widest opacity-40 text-[var(--primary)]">চিকিৎসা</span>
                      </div>
                      <p className="text-sm font-bold leading-relaxed">{disease.solution.join(". ")}।</p>
                    </div>
                    ...
                    <div className="p-4 rounded-2xl bg-cyan-400/5 border border-cyan-400/10 flex flex-wrap gap-2 items-center">
                      ...
                      {disease.medicines.map((m, i) => (
                        <span key={i} className="text-[10px] font-black text-cyan-400 bg-cyan-400/10 px-3 py-1.5 rounded-lg border border-cyan-400/20 uppercase">
                          {m}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-2 opacity-20 group-hover:opacity-100 transition-opacity">
                    <Info size={14} />
                    <p className="text-[10px] font-bold uppercase tracking-widest leading-none">বিশেষজ্ঞের পরামর্শ বাধ্যতামূলক</p>
                  </div>
                </div>
              </motion.div>
            ))
