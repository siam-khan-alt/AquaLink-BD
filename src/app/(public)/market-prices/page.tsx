...
export const metadata: Metadata = {
  ...
  description:
    "ঢাকার পাইকারি ও খুচরা বাজারের মাছের সর্বশেষ সঠিক বাজার দর জানুন।",
};

export const revalidate = 1800;

export default async function MarketPricesPage() {
  let data: IMarketPrice[] = [];
  ...
  const upTrendCount = data.filter(
    (fish: IMarketPrice) => fish.trend === "up"
  ).length;
  ...
  return (
    <main className="min-h-screen py-10 container mx-auto px-4">
      ...
      <section className="bg-[var(--surface)]/30 p-2 rounded-2xl border border-[var(--border)]/50 shadow-inner">
        {dbError ? (
          <div className="rounded-3xl border border-red-500/20 bg-red-500/10 p-6 text-red-900">
            <p className="font-semibold">ডাটাবেস সংযোগে সমস্যা হয়েছে।</p>
            <p className="text-sm opacity-70">{dbError}</p>
            <p className="mt-2 text-sm opacity-70">
              অনুগ্রহ করে পরে আবার চেষ্টা করুন বা আপনার ডাটাবেস সেটিংস চেক করুন।
            </p>
          </div>
        ) : (
...
const StatCard = ({ icon, label, value, className = "" }: StatCardProps) => (
  <div
    className={`bg-[var(--surface)] border border-[var(--border)] p-5 rounded-2xl shadow-sm hover:border-[var(--primary)]/30 transition-all group ${className}`}
  >
